...
Creating a new Buyer or Pro account now creates an unverified local prototype account. Verification must be submitted through Readiness and approved by the appropriate Admin flow.
...
This is a frontend-only, private-beta product prototype. Before production, replace seeded local state with authenticated backend services, a compliant identity/KYC vendor, controlled media storage, production payment-provider integrations, observability, and role-enforced server authorization.
